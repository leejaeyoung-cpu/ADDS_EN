...
class CDSSRealDataTester:
    ...
    def test_cellpose_integration(self):
        ...
        print(f"[PASS] Loaded sample: {patient.get('patient_id')}")
        print(f"   Cell count: {cellpose_results.cell_count}")
        print(f"   Mean area: {cellpose_results.mean_area_um2:.1f} um2")
    ...
    def test_ct_integration(self):
        ...
        if not dicom_dir.exists():
            print(f"[FAIL] DICOM directory not found: {dicom_dir}")
            return None
        
        dicom_files = list(dicom_dir.glob("*.dcm"))
        print(f"[PASS] Found {len(dicom_files)} DICOM files")
        
        if len(dicom_files) == 0:
            print("[FAIL] No DICOM files found")
            return None
    ...
    def test_clinical_data(self):
        ...
        print(f"[PASS] Patient ID: {clinical_data.patient_id}")
    ...
    def save_test_results(self, profile=None):
        ...
        if profile:
            ...
            print(f"[SAVED] Patient profile: {profile_file}")
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            if not cellpose_results:
                print("[FAIL] Cellpose test failed")
                return
            
            # Test 2: CT Detection
            ct_results = self.test_ct_integration()
            if not ct_results:
                print("[FAIL] CT detection test failed")
                return
            
            # Test 3: Clinical Data
            clinical_data = self.test_clinical_data()
            if not clinical_data:
                print("[FAIL] Clinical data test failed")
                return
...
def main():
    ...
    if profile:
        print("\n[SUCCESS] All tests passed! Real data integration is working.")
        return 0
    else:
