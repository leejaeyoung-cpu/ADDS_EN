"""
figure_v6.py  ──  Final production version
════════════════════════════════════════════════════════════════
Layout strategy: BOTTOM-UP anchoring
  1. Footer strip at y=0.20
  2. Output box just above footer (y=0.48)
  3. ADDS sits on top of output box + small gap (arrow zone)
  4. 3 modules stacked upward from ADDS top
  5. Team headers sit on top of modules
  6. Timeline row above team headers
  7. Title at very top

Connectors: explicit bend columns (never inside any box).
Feedback loop: right outer margin only.
All labels: clear of boxes.
"""
...
# ═══════════════════════════════════════════════════════════════
# GRID (BOTTOM-UP) — compute FH dynamically
# ═══════════════════════════════════════════════════════════════
# Column widths
LX, LW = 0.5, 7.2
RX, RW = 14.3, 7.2
...
# Bottom-up
FOOT_Y   = 0.0
OUT_Y    = FOOT_Y + FOOT_H + 0.04
ADDS_BOT = OUT_Y + OUT_H + ARW_ZONE
ADDS_TOP = ADDS_BOT + ADDS_H

MGAP_ABOVE = 0.08
mods_bot = []
cur = ADDS_TOP + MGAP_ABOVE
for i in range(N_MOD):
    by = cur
    mods_bot.append((by, by + M_H/2))
    cur = by + M_H + M_GAP

HIGHEST_MOD_TOP = mods_bot[-1][0] + M_H
THEAD_Y = HIGHEST_MOD_TOP + 0.08
TLINE_Y = THEAD_Y + THEAD_H + 0.08
TITLE_Y = TLINE_Y + TLINE_H + 0.08

# Compute FH from content (no top white-space)
FW = 22
FH = TITLE_Y + TITLE_H + TOP_PAD
...
P = {
    "bg":   "#F8FAFC",
    "txt2": "#374151", "txt3": "#94A3B8", "border": "#CBD5E1",
...
def av(x, y0, y1, c, lw=2.5, z=4):
    ax.annotate("", xy=(x, y1), xytext=(x, y0),
                arrowprops=dict(arrowstyle="-|>,head_width=0.44,head_length=0.54",
...
# 2. TIMELINE CHEVRONS
# ═══════════════════════════════════════════════════════════════
CH = 0.26   # chevron notch
phases = [
    (LX,          7.15,          P["b1"],
     "1–3차년도  |  Phase 1–3",    "세포주 기반 기초 기전 규명  ·  In-Vitro Mechanistic Studies"),
    (LX+7.38,     7.10,          P["v1"],
     "4차년도  |  Phase 4",        "임상 검체 + PDO 구축  ·  Biospecimen & Organoid"),
    (LX+14.75,    FW-LX-14.75-0.35, P["g1"],
     "5–6차년도  |  Phase 5–6",   "ADDS 최적화 + 전임상 검증  ·  Validation"),
]
for i, (px, pw, pc, pl, pd) in enumerate(phases):
    rb(px, TLINE_Y, pw, TLINE_H, pc, "none", lw=0, r=0.07)
    T(px+pw/2, TLINE_Y+TLINE_H*0.70, pl, sz=9.5, c="white", bold=True)
    T(px+pw/2, TLINE_Y+TLINE_H*0.28, pd, sz=8.5, c="white")
    if i < 2:
        ax.add_patch(mpatches.Polygon(
            [[px+pw, TLINE_Y], [px+pw+CH, TLINE_Y+TLINE_H/2], [px+pw, TLINE_Y+TLINE_H]],
            facecolor=P["bg"], edgecolor="none", zorder=3))

# ═══════════════════════════════════════════════════════════════
# 3. TEAM HEADERS
# ═══════════════════════════════════════════════════════════════
for tx, tw, tc, nm, pi, aff in [
    ...
    T(tx+tw/2, THEAD_Y+THEAD_H-0.36, nm, sz=13, c="white", bold=True)
    T(tx+tw/2, THEAD_Y+THEAD_H-0.70, pi, sz=9.5, c="white")
    hl(THEAD_Y+0.52, tx+0.4, tx+tw-0.4, c=(1,1,1,0.35), lw=0.8)
    T(tx+tw/2, THEAD_Y+0.25, aff, sz=8.5, c="white")

# ═══════════════════════════════════════════════════════════════
# 4. MODULE CARDS
# ═══════════════════════════════════════════════════════════════
# Note: mods_bot[0]=lowest slot(nearest ADDS), mods_bot[2]=highest slot
# So list index 0 → displayed at BOTTOM of module column (directly above ADDS)
# List should be ordered: top-most module first, reversed for placement
LEFT_MODS_ORDER = [
...
# Reverse so that index 0 = bottom slot (nearest ADDS) = Phase1-3 first module
LEFT_MODS  = list(reversed(LEFT_MODS_ORDER))
RIGHT_MODS = list(reversed(RIGHT_MODS_ORDER))

left_cy  = []
right_cy = []

for i, ((lt, ld), (rt, rd)) in enumerate(zip(LEFT_MODS, RIGHT_MODS)):
    # mods_bot[0] = lowest module (nearest ADDS), mods_bot[2] = highest
    by, bcy = mods_bot[i]
...
# 5. ADDS PANEL
# ═══════════════════════════════════════════════════════════════
rb(AX, ADDS_BOT, AW, ADDS_H, P["v3"], P["v1"], lw=2.0, r=0.14, z=2)

# Title ribbon at top of ADDS
RIB_H = 0.72
rb(AX, ADDS_TOP - RIB_H, AW, RIB_H, P["v0"], "none", lw=0, r=0.12, z=3)
ax.add_patch(mpatches.Rectangle((AX, ADDS_TOP-RIB_H), AW, RIB_H*0.42,
                                  facecolor=P["v0"], edgecolor="none", zorder=3))
T(AC, ADDS_TOP - RIB_H/2,
  "ADDS 플랫폼  |  AI-Driven Drug Discovery & Synergy System",
  sz=10.5, c="white", bold=True)

# 3 step boxes — evenly distributed
STEPS = [
...
for j, (stitle, sdesc) in enumerate(STEPS):
    ...
    rb(AX+0.30, bby+STEP_H/2-0.24, 0.48, 0.48, P["v1"], "none", lw=0, r=0.07, z=4)
    T(AX+0.54, bby+STEP_H/2, str(j+1), sz=11, c="white", bold=True)
    T(AX+0.86+(AW-0.86)/2, bby+STEP_H-0.28, stitle, sz=10, bold=True, c=P["v0"])
    T(AX+0.86+(AW-0.86)/2, bby+0.38, sdesc, sz=8.5, c=P["txt2"])
    if j < len(STEPS)-1:
        av(AC, bby - 0.01, bby - STEP_GAP + 0.01, c=P["v1"], lw=2)
    sy = bby - STEP_GAP

# ADDS → Output arrow (fits exactly in ARW_ZONE)
av(AC, ADDS_BOT - 0.02, OUT_Y + OUT_H + 0.02, c=P["v1"], lw=4.5, z=4)
...
T(FW/2, OUT_Y+OUT_H-0.32,
  ...
  sz=12, c="white", bold=True)
hl(OUT_Y+OUT_H-0.56, 0.90, FW-0.90, c=P["v2"], lw=0.9)
...
for k, otxt in enumerate(outs):
    ox = OX_S + k*(OW_E+0.15)
    rb(ox, OUT_Y+0.07, OW_E, OUT_H-0.66, P["v1"], "none", lw=0, r=0.09, z=3)
    T(ox+OW_E/2, OUT_Y+0.07+(OUT_H-0.66)/2, otxt, sz=8.5, c="white", ls=1.5)

# ═══════════════════════════════════════════════════════════════
# 7. CONNECTORS (modules → ADDS)
# ═══════════════════════════════════════════════════════════════
BL = AX - 0.36   # bend column left of ADDS
BR = AX + AW + 0.36

for i, (ly, ry, py) in enumerate(zip(left_cy, right_cy, step_ports)):
    # Left: right edge of module → BL → horizontal → ADDS left wall
    ax.plot([LX+LW, BL], [ly, ly], color=P["b1"], lw=2.0, zorder=4, alpha=0.85)
    ax.plot([BL, BL],    [ly, py], color=P["b1"], lw=2.0, zorder=4, alpha=0.85)
    ah(py, BL, AX - 0.01, c=P["b1"], lw=2.0, z=4)

    # Right: left edge of module → BR → horizontal → ADDS right wall
    ax.plot([RX, BR],    [ry, ry], color=P["g1"], lw=2.0, zorder=4, alpha=0.85)
    ax.plot([BR, BR],    [ry, py], color=P["g1"], lw=2.0, zorder=4, alpha=0.85)
    ah(py, BR, AX + AW + 0.01, c=P["g1"], lw=2.0, z=4)

# Connector labels (placed safely outside any box)
T(BL - 0.22, step_ports[0] + 0.26, "In-Vitro\nData", sz=7.5, c=P["b1"], bold=True, ha="right")
T(BR + 0.22, step_ports[0] + 0.26, "Clinical\nData",  sz=7.5, c=P["g1"], bold=True, ha="left")

# ═══════════════════════════════════════════════════════════════
# 8. FEEDBACK LOOP
# ═══════════════════════════════════════════════════════════════
FB_X = FW - 0.36
fb_y_start = OUT_Y + OUT_H - 0.38
fb_y_end   = THEAD_Y + THEAD_H/2

hl(fb_y_start, FW - 0.50, FB_X, c=P["g1"], lw=2.0, ls="--")
vl(FB_X, fb_y_start, fb_y_end, c=P["g1"], lw=2.0, ls="--")
ah(fb_y_end, FB_X, RX+RW - 0.01, c=P["g1"], lw=2.0)
T(FB_X - 0.18, OUT_Y + OUT_H + 0.18,
  "임상 피드백 루프 / Clinical Feedback Loop",
...
ax.legend(handles=leg, loc="lower left", bbox_to_anchor=(0.012, 0.002),
          fontsize=9, framealpha=0.95, edgecolor=P["border"], ncol=4,
          handleheight=1.0, handlelength=1.5, columnspacing=0.9, fancybox=True)

T(FW/2, FOOT_Y + FOOT_H*0.38,
  "2026년도 미래도전연구지원사업 신규과제  |  인하대학교 의과대학  |  이상훈·최문석 공동연구팀",
  sz=8.5, c=P["txt3"])

# ─────────────────────────────────────────────────────────────
out = r"f:\ADDS\docs\figure_v6.png"
