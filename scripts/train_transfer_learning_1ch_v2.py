"""
개선된 Transfer Learning - V2
- 95% tumor-focused sampling
- Weighted loss for class imbalance
- Only tumor-containing samples
"""
...
def main():
    config = {
        ...
        'batch_size': 2,
        'num_epochs': 50,
        'learning_rate': 0.0002,  # Slightly higher than before
        'device': 'cuda',
        'save_dir': 'models/transfer_learning_1ch_v2',
        'pretrained_path': 'models/pretrained/swin_unetr_pretrained.pt'
    }
    
    logger.info("="*60)
    logger.info("[1-Channel Transfer Learning V2]")
    logger.info("95% Tumor-Focused + Weighted Loss")
    logger.info("="*60)
    ...
    # Dataset V2 - improved sampling
    logger.info("\n[Creating V2 Dataset]")
    train_dataset = ColonCancerDataset1ChannelV2(
        config['data_root'], fold=config['fold'], mode='train', tumor_focused_prob=0.95
    )
    val_dataset = ColonCancerDataset1ChannelV2(
        config['data_root'], fold=config['fold'], mode='val', tumor_focused_prob=0.0
    )
    ...
    # Training components - MORE aggressive focal loss
    criterion = FocalDiceCELoss(0.6, 0.3, 0.1, 0.8, 3.0)  # Higher focal weight & gamma
